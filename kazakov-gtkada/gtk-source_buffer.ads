...
package Gtk.Source_Buffer is
   ...
   Case_Insensitive : constant Gtk_Text_Search_Flags := 2**2;
--
...
--
   procedure Backward_Iter_To_Source_Mark
             (  Buffer   : access Gtk_Source_Buffer_Record;
...
--  [ Limit ]     - Bound for the search (the buffer start by default)
--    Result      - True if found (Match_Start and Match_End are set)
--
-- This  procedure  searches backward  for Str. Any match is returned by
-- setting Match_Start to the first character of the match and Match_End
...
-- If  the  Visible_Only  flag  is present, the match may have invisible
-- text interspersed in Str. i.e. Str will be  a  possibly-noncontiguous
-- subsequence   of   the  matched  range.  similarly,  if  you  specify
-- Text_Only,  the  match may have pixbufs or child widgets mixed inside
-- the matched range. If these flags are not given, the  match  must  be
-- exact;  the  special  16#FFFC#  character  in Str will match embedded
-- pixbufs or child widgets. If you specify the  Case_Insensitive  flag,
...
--    Match_Start - Return location for start of match
--    Match_End   - Return location for end of match
--  [ Limit ]     - Bound for the search (the buffer end by default)
--    Result      - True if found (Match_Start and Match_End are set)
--
-- This  procedure  searches  forward  for Str. Any match is returned by
-- setting Match_Start to the first character of the match and Match_End
-- to  the first character after the match. The search will not continue
-- past  Limit. Note that a search is a linear or O(n) operation, so you
-- may wish to use limit to avoid locking up your UI on  large  buffers.
...
--
   procedure Gtk_New
             (  Buffer   : out Gtk_Source_Buffer;
                Language : access Gtk_Source_Language_Record'Class
             );
--
-- Gtk_New -- Object creation
--
...
--
   procedure Initialize
             (  Buffer   : access Gtk_Source_Buffer_Record'Class;
                Language : access Gtk_Source_Language_Record'Class
             );
--
-- Initialize -- To be called by any derived object
...
--    Buffer - The buffer
--
   procedure Redo (Buffer : access Gtk_Source_Buffer_Record);
--
...
--
   procedure Set_Style_Scheme
             (  Buffer : access Gtk_Source_Buffer_Record;
                Scheme : access Gtk_Source_Style_Scheme_Record'Class
             );
--
...
--
   procedure Undo (Buffer : access Gtk_Source_Buffer_Record);

private

   type Gtk_Source_Buffer_Record is
      new Gtk_Text_Buffer_Record with null record;
