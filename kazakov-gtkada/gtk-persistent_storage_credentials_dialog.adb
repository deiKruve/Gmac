...
package body Gtk.Persistent_Storage_Credentials_Dialog is
   ...
   Credentials_Dialog_Class_Record : GObject_Class :=
      Uninitialized_Class;
   ...
   begin
      ...
      Gtk.Object.Checked_Destroy (Dialog);
   exception
      when others =>
         if Dialog /= null then
            Gtk.Object.Checked_Destroy (Dialog);
   ...
   begin
      if Dialog.Error_Label = null then
         Gtk_New (Frame);
         Set_Border_Width (Frame, Style_Get (Dialog, "row-spacing"));
         Gtk_New (Dialog.Error_Label, Text);
         Add (Frame, Dialog.Error_Label);
         Set_Alignment (Dialog.Error_Label, 0.0, 0.5);
         Set_Justify (Dialog.Error_Label, Justify_Left);
         Set_Property
         (  Dialog.Error_Label,
   ...
   begin
      if Scheme /= DSN_Scheme then
         Raise_Exception (Status_Error'Identity, "Not a DSN scheme");
      end if;
   ...
   begin
      return Get_Text (Dialog.Name_Entry);
   end Get_Name;
   ...
   begin
      return Get_Text (Dialog.User_Entry);
   end Get_User;
   ...
   begin
      return Get_Active (Dialog.Store_Password);
   ...
   begin
      Initialize (Dialog, "", User, Stored);
      Set_Sensitive (Dialog.Name_Entry, True);
   end Initialize;
   ...
   procedure Initialize
      ...
      To_Install : constant Boolean :=
          Credentials_Dialog_Class_Record = Uninitialized_Class;
   begin
      Gtk.Dialog.Initialize (Dialog);
      Initialize_Class_Record
      (  Dialog,
         Null_Array,
         Credentials_Dialog_Class_Record,
      ...
      (  Dialog,
         "style-set",
         Dialog_Handlers.To_Marshaller (Style_Set'Access)
      );

      Gtk_New (Dialog.Grid, 4, 2, False);

      Gtk_New (Dialog.Name_Label, "");
      Attach (Dialog.Grid, Dialog.Name_Label, 0, 1, 0, 1, Fill, 0);
      Set_Justify (Dialog.Name_Label, Justify_Right);
      Set_Alignment (Dialog.Name_Label, 1.0, 0.5);
      Gtk_New (Dialog.User_Label, "");
      Attach (Dialog.Grid, Dialog.User_Label, 0, 1, 1, 2, Fill, 0);
      Set_Justify (Dialog.User_Label, Justify_Right);
      Set_Alignment (Dialog.User_Label, 1.0, 0.5);
      Gtk_New (Dialog.Password_Label, "");
      Attach (Dialog.Grid, Dialog.Password_Label, 0, 1, 2, 3, Fill, 0);
      Set_Justify (Dialog.Password_Label, Justify_Right);
      Set_Alignment (Dialog.Password_Label, 1.0, 0.5);
      ...
      Pack_Start (Get_Vbox (Dialog), Dialog.Grid);
      Show_All (Dialog.Grid);

      Set_Text (Dialog.Name_Entry, Name);
      Set_Text (Dialog.User_Entry, User);
      Set_Active (Dialog.Store_Password, Stored);
      ...
      loop
         case Run (Dialog) is
            when Gtk_Response_OK =>
               declare
                  Name : UTF8_String := Get_Name (Dialog);
               begin
               ...
               exception
                  ...
                  when Reason : others =>
                     Error (Dialog, Exception_Message (Reason));
               end;
            when others =>
               declare
   ...
   begin
      Set_Title (Dialog, Style_Get (Dialog, "title"));
      Set_Text (Dialog.Name_Label, Style_Get (Dialog, "name-label"));
      Set_Text (Dialog.User_Label, Style_Get (Dialog, "user-label"));
      Set_Text
      (  Dialog.Password_Label,
